#==simple plots from a single run

  inDir<-'SYC_2_allGAMs_2/10/97'
  #inDir<-'SYC_1_noTV_1/1/97'
 # inDir<-'SYC_3_changeM_2/1/97'
  
  
  REP<-readLines(file.path(inDir,"/simass.rep"))
  CTL<-readLines(file.path(inDir,"/simass.CTL"))
  DAT<-readLines(file.path(inDir,"/simass.DAT"))
  TRU<-readLines(file.path(inDir,"/TrueQuantities.DAT"))
  data2<-scan(file.path(inDir,"/simass.PAR"),what="character")
