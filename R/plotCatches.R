...
catchdat <- read.csv(file.path(dir.data,"ECS_SYC_catches.csv"),header=T)
...
ggplot(data = catchdat) + 
  geom_col(mapping = aes(x = Year, y = Catch)) +
...
preddat <- read_csv(file.path(dir.data,"SYC_predators.csv"))
