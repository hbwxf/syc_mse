...
#   	barplot(rbind(projCatLenFreqN[i,,1]),beside=T,xaxt='n',yaxt='n')
	...
	return(as.vector(trueCatchN))
}


Find_Rzero<-function(logRvec,trueCatch=tCatch,Nyrs=Nyears,Cind=Cindex,OMname=newOM) {
	Rvec <- exp(logRvec^2)
	smoothingLL <-1E-8*sqrt(sum(diff(Rvec,lag=1)^2))
	InitYr <- OMname$OM$InitYear

	OMname$OM$RzeroN <- c(rep(Rvec[1],(InitYr-length(Rvec))),Rvec,rev(Rvec)[1])

	estCatch <- miniGeMS(OMname)[Cind]
...
res<-nlminb(tempRvec,Find_Rzero)

res2 <- res
for(i in 1:100000) {
	res1 <- res
#	res <- optim(res$par,Find_Rzero,method="L-BFGS-B")
	res<-nlminb(res$par,Find_Rzero)
...
#	cat(paste0(res$value,"\n"))
	...
	newOM$OM$RzeroN <- c(rep(Rvec[1],(newOM$OM$InitYear-length(Rvec))),Rvec,rev(Rvec)[1])
	par(mfrow=c(2,1),mar=c(1,1,1,1),oma=c(3.3,3.3,1,1),xpd=NA)
	plot(as.vector(miniGeMS(newOM)),type="l",ylab="SYC Catch", xlab="",xaxt="n")
...
Rvec <- exp(res$par^2)
paste0("c(",paste(c(rep(Rvec[1],(newOM$OM$InitYear-length(Rvec))),Rvec,rev(Rvec)[1]),collapse=","),")")
